...
import torch
import numpy as np
from rdkit.Chem import Draw, AllChem, Crippen, QED, Descriptors
from rdkit import Chem, DataStructs

from mflow_data import transform_qm9, transform_zinc250k
from mflow_data.transform_zinc250k import zinc250_atomic_num_list, transform_fn_zinc250k
...
import cairosvg
from model.gin_model import GNN_generate
import time
...
from transformers import AutoTokenizer, BertConfig, BertTokenizer, BertForPreTraining

from model.bert import MMBert
...
def generate_mols_fix(model, atomic_num_list, temp=0.7, z_mu=None, batch_size=20, true_adj=None, device=-1):  #  gpu=-1):
    ...
    datamol = MolTransfer(x0.detach(), adj0.detach(), atomic_num_list)
...
def tokenizer_text(text, text_max_len):
    ...
    input_ids[:, 0] = tokenizer.enc_token_id
    return input_ids, attention_mask


def run_z_optimize(model, atomic_num_list, graph_encoder, text_encoder, gtm_head, input_text, z, device, num_steps=100, g_emb=None, g_mask=None):

    optimizer = torch.optim.Adam([z.requires_grad_()], lr=1e-2)
    input_ids, attention_mask = tokenizer_text(input_text, text_max_len=128)
    ...
    while step <= num_steps:
        ...
        xs = x0.softmax(dim=1)
        try:
            node_rep, mask, graph_rep = graph_encoder(xs, edge_attr, datamol, atomic_num_list, device)
        except:
            print('Cannot encode the graph!!!')
            break
        g_emb = torch.cat([graph_rep.unsqueeze(1), node_rep], dim=1)
        g_mask = torch.cat([torch.ones(mask.size()[:-1], dtype=torch.long).unsqueeze(1).to(device), mask], dim=1)

        output = text_encoder.text_encode(input_ids, attention_mask, g_emb, g_mask)
        gl_output = gtm_head(output)
        loss = F.cross_entropy(gl_output, torch.ones(1, dtype=torch.long).to(device))
        if torch.isnan(loss):
            print(loss)
            break
        loss.backward()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_dir", type=str, default='mflow/results/zinc250k_512t2cnn_256gnn_512-64lin_10flow_19fold_convlu2_38af-1-1mask')
    parser.add_argument("--data_dir", type=str, default='mflow_data')
    ...
    parser.add_argument('--n_experiments', type=int, default=5, help='number of times generation to be run')
    ...
    ckpt=load_model(snapshot_path, model_params).state_dict()
    ...
    ckpt = torch.load("checkpoints/MoLa/epoch=99.ckpt", map_location=device)
    ckpt = ckpt['state_dict']

    # load graph encoder
    graph_encoder = GNN_generate(num_layer=5,
                                 emb_dim=300,
                                 gnn_type='gin',
                                 drop_ratio=0.0,
                                 JK='last',
    ...
    config = BertConfig(vocab_size=31093, )
    config.add_cross_attention = True
    config.graph_dim = 300
    text_encoder = MMBert(config=config)

    gtm_head = nn.Sequential(
        nn.Linear(768, 768),
    ...
    for k, v in ckpt.items():
        if "text_encoder.main_model." in k:
            bert_pretrained_dict[k[24:]] = v
    ...
    for k, v in ckpt.items():
        if "gtm_head." in k:
            gtm_head_dict[k[9:]] = v
    missing_keys, unexpected_keys = gtm_head.load_state_dict(gtm_head_dict, strict=False)
    ...
    for name, param in gtm_head.named_parameters():
        param.requires_grad = False
    ...
    input_text_list = [
                       'The molecule contains amine, hydroxyl and carbonyl groups.',
                       'The molecule is insoluble in water and reactive with active metals.',
                       ]
    g_num = 100
    ...
    for j in range(text_num):
        ...
        all_seeds = np.zeros((g_num, z_dim))
        for i in range(g_num):
            ...
            z, loss = run_z_optimize(model, atomic_num_list, graph_encoder, text_encoder, gtm_head, input_text, z, device)
            ...
            if save_fig:
                ...
                filepath = os.path.join(gen_dir, 'opt_mols_{}.png'.format(i))
                img = Draw.MolsToGridImage(val_res['valid_mols'], legends=val_res['valid_smiles'], molsPerRow=1, subImgSize=(300, 300))
                img.save(filepath)

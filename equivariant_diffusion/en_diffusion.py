...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def phi(self, x, t, node_mask, edge_mask, reference):
        net_out = self.dynamics._forward(t, x, node_mask, edge_mask, reference)
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, reference, fix_noise=False):
        """Samples x ~ p(x|z0)."""
        zeros = torch.zeros(size=(z0.size(0), 1), device=z0.device)
        gamma_0 = self.gamma(zeros)
        # Computes sqrt(sigma_0^2 / alpha_0^2)
        sigma_x = self.SNR(-0.5 * gamma_0).unsqueeze(1)
        net_out = self.phi(z0, zeros, node_mask, edge_mask, reference)
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, reference, t0_always):
        ...
        net_out = self.phi(z_t, t, node_mask, edge_mask, reference)
        ...
        if t0_always:
            ...
            net_out = self.phi(z_0, t_zeros, node_mask, edge_mask, reference)
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, reference=None):
        """
        ...
        if self.training:
            ...
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, reference, t0_always=False)
        else:
            # Less variance in the estimator, costs two forward passes.
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, reference, t0_always=True)
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, reference, fix_noise=False):
        ...
        # Neural net prediction.
        eps_t = self.phi(zt, t, node_mask, edge_mask, reference)
    ...
    @torch.no_grad()
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, reference, fix_noise=False):
        """
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, reference, fix_noise=fix_noise)

        # Finally sample p(x, h | z_0).
        x, h = self.sample_p_xh_given_z0(z, node_mask, edge_mask, reference, fix_noise=fix_noise)
    ...
    @torch.no_grad()
    def sample_chain(self, n_samples, n_nodes, node_mask, edge_mask, reference, keep_frames=None):
        """
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(
                s_array, t_array, z, node_mask, edge_mask, reference)
        ...
        # Finally sample p(x, h | z_0).
        x, h = self.sample_p_xh_given_z0(z, node_mask, edge_mask, reference)

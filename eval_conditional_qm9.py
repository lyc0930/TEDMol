...
class DiffusionDataloader:
    ...
    def sample(self):
        ...
        reference = self.prop_dist.sample_batch(nodesxsample).to(self.device)
        ...
        if self.unkown_labels:
            reference[:] = self.prop_dist.normalizer[prop_key]['mean']
        else:
            reference = reference * self.prop_dist.normalizer[prop_key]['mad'] + self.prop_dist.normalizer[prop_key]['mean']
        data = {
            'positions': x.detach(),
            'atom_mask': node_mask.detach(),
            'edge_mask': edge_mask.detach(),
            'one_hot': one_hot.detach(),
            prop_key: reference.detach()
        }
        return data
...
def main_quantitative(args):
    ...
    property_norms = compute_mean_mad(dataloaders, args_gen.condition, args_gen.dataset)

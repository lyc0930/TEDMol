...
import time
import pickle
from qm9.utils import prepare_reference, compute_mean_mad
from train_test import train_epoch, test, analyze_and_save

parser = argparse.ArgumentParser(description='TEDMol')
parser.add_argument('--exp_name', type=str, default='QM9')
...
parser.add_argument('--dataset', type=str, default='qm9', help='qm9 | qm9_second_half (train only on the last 50K samples of the training dataset)')
...
parser.add_argument('--n_stability_samples', type=int, default=1000, help='Number of samples to compute the stability')
...
if len(args.condition) > 0:
    print(f'Conditioning on {args.condition}')
    property_norms = compute_mean_mad(dataloaders, args.condition, args.dataset)
    reference_dummy = prepare_reference(args.condition, data_dummy, property_norms)
    reference_node_nf = reference_dummy.size(2)
else:
    reference_node_nf = 0
    property_norms = None

args.reference_node_nf = reference_node_nf

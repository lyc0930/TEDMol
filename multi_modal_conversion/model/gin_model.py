...
class GINConv_generate(MessagePassing):
    """
    Extension of GIN aggregation to incorporate edge information by concatenation.
    ...
    def __init__(self, emb_dim, aggr="add"):
        super(GINConv_generate, self).__init__(aggr="add")
        # multi-layer perceptron
    ...
    def forward(self, x, edge_index, edge_attr, edge_attr0=None, device=torch.device('cpu')):
        ...
        self_loop_attr = torch.ones(x.size(0), 1).to(device)
        self_loop_attr0 = torch.mm(self_loop_attr, self.edge_embedding1(torch.arange(4, 5).to(device)))
...
class GNN_generate(torch.nn.Module):
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0, gnn_type="gin"):
        super(GNN_generate, self).__init__()
        ...
        for layer in range(num_layer):
            if gnn_type == "gin":
                self.gnns.append(GINConv_generate(emb_dim, aggr="add"))
            elif gnn_type == "gcn":
    ...
    def forward(self, *argv):
        ...
        x = torch.mm(x0[:, 0:num_ato], self.x_embedding1(torch.tensor(atomic_num_list0).to(device) - 1)) + torch.mm(
            x0[:, -1:], self.x_embedding1(torch.arange(119, 120).to(device))) + self.x_embedding2(x[:, 1])

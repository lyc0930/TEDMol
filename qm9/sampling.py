...
def sample_chain(args, device, flow, n_tries, dataset_info, prop_dist=None):
    ...
    if args.reference_node_nf > 0:
        reference = prop_dist.sample(n_nodes).unsqueeze(1).unsqueeze(0)
        reference = reference.repeat(1, n_nodes, 1).to(device)
        #reference = torch.zeros(n_samples, n_nodes, args.reference_node_nf).to(device)
    else:
        reference = None
    ...
    if args.probabilistic_model == 'diffusion':
        ...
        for i in range(n_tries):
            chain = flow.sample_chain(n_samples, n_nodes, node_mask, edge_mask, reference, keep_frames=100)
...
def sample(args, device, generative_model, dataset_info,
           prop_dist=None, nodesxsample=torch.tensor([10]), reference=None,
           fix_noise=False):
    ...
    if args.reference_node_nf > 0:
        if reference is None:
            reference = prop_dist.sample_batch(nodesxsample)
        reference = reference.unsqueeze(1).repeat(1, max_n_nodes, 1).to(device) * node_mask
    else:
        reference = None

    if args.probabilistic_model == 'diffusion':
        x, h = generative_model.sample(batch_size, max_n_nodes, node_mask, edge_mask, reference, fix_noise=fix_noise)
...
def sample_sweep_conditional(args, device, generative_model, dataset_info, prop_dist, n_nodes=19, n_frames=100):
    ...
    for key in prop_dist.distributions:
        ...
        reference_row = torch.tensor(np.linspace(min_val, max_val, n_frames)).unsqueeze(1)
        reference.append(reference_row)
    reference = torch.cat(reference, dim=1).float().to(device)

    one_hot, charges, x, node_mask = sample(args, device, generative_model, dataset_info, prop_dist, nodesxsample=nodesxsample, reference=reference, fix_noise=True)
    return one_hot, charges, x, node_mask